...
(function() {
  function L(a) {
    ...
    function h(a) {
      for (
        ...
        if (/\\[bdsw]/i.test(j)) a.push(j);
          ...
          d < 65 ||
            j > 122 ||
            (d < 65 || j > 90 || b.push([Math.max(65, j) | 32, Math.min(d, 90) | 32]), d < 97 || j > 122 || b.push([Math.max(97, j) & -33, Math.min(d, 122) & -33]));
        }
      }
      b.sort((a, f) => {
    ...
    function y(a) {
      for (
        var f = a.source.match(/\[(?:[^\\\]]|\\[\S\s])*]|\\u[\dA-Fa-f]{4}|\\x[\dA-Fa-f]{2}|\\\d+|\\[^\dux]|\(\?[!:=]|[()^]|[^()[\\^]+/g), b = f.length, d = [], c = 0, i = 0;
        c < b;
        ++c
      ) {
        var j = f[c];
        j === '(' ? ++i : j.charAt(0) === '\\' && (j = Number(j.substring(1))) && j <= i && (d[j] = -1);
      }
      for (c = 1; c < d.length; ++c) d[c] === -1 && (d[c] = ++t);
      for (i = c = 0; c < b; ++c)
        (j = f[c]), j === '(' ? (++i, d[i] === void 0 && (f[c] = '(?:')) : j.charAt(0) === '\\' && (j = Number(j.substring(1))) && j <= i && (f[c] = '\\' + d[i]);
    ...
    for (var r = { b: 8, t: 9, n: 10, v: 11, f: 12, r: 13 }, n = [], p = 0, d = a.length; p < d; ++p) {
      g = a[p];
      if (g.global || g.multiline) throw new Error(String(g));
      n.push('(?:' + y(g) + ')');
    }
    return new RegExp(n.join('|'), l ? 'gi' : 'g');
  }
  function M(a) {
    function m(a) {
      switch (a.nodeType) {
  ...
  function D(a, m) {
    ...
    for (var n = Math.max(0, (m - 1) | 0) || 0, g = 0, z = d.length; g < z; ++g)
      (l = d[g]), (l.className = 'L' + ((g + n) % 10)), l.firstChild || l.appendChild(s.createTextNode('\u00A0')), r.appendChild(l);
    a.appendChild(r);
  }
  function k(a, m) {
