...
class Platform {
   ...
   * @deprecated Since 3.0.1. Use Platform.privatehive.listChannelInvites
   * @returns {Array<Invite>} List of invites
   * @memberof Platform
   * @instance
   */
  async listPrivatehiveChannelInvites() {
    console.warn('Deprecation: Platform.listPrivatehiveChannelInvites is deprecated. Use Platform.privatehive.listChannelInvites');
    const path = '/api/platform/privatehive/invite';
    try {
      const res = await helpers.sendPlatformRequest({ domain: this.domain, path, method: 'GET', apiKey: this.apiKey });
      return res.data;
