...
class Platform {
  ...
  async listPrivatehiveNetworks(instanceId, options) {
    ...
    try {
      const res = await helpers.sendPlatformRequest({ path, method: 'GET', apiKey: this.apiKey });
      return res.data;
    } catch (err) {
      return Promise.reject(err);
    }
  ...
  async acceptPrivatehiveChannelInvite(inviteId, peerId) {
    const path = `/privatehive/invite/accept/${inviteId}`;
    const body = {
      peerId,
    };
    try {
