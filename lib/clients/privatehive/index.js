 ...
 */
class PrivatehiveNode {
  constructor({ name, instanceId, orgName, locationCode, domain, type, ordererURL, ...details }) {
    this.name = name;
  ...
  setOrderer(orderer) {
    ...
    this.orderer.ordererURL = `${orderer.workerNodeIP}:${orderer.ordererNodePort}`;
  }
  ...
  async addChaincode(filePath, name, type) {
    ...
    const addReq = this.__constructNodeRequest('/chaincodes/add', RequestTypes.POST);
  ...
  async fetchChaincodes() {
    return this.__constructNodeRequest('/chaincodes/list', RequestTypes.GET);
  }
