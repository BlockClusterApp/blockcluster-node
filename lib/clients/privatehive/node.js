...
class PrivatehiveNode {
  constructor({ name, instanceId, orgnization, locationCode, domain, type, ordererURL }) {
    this.name = name;
   ...
   * @return {void}@memberof PrivatehiveNode
   */
  setOrderer(orderer) {
    if (this.type === 'peer') {
   ...
   */
  async createChannel(name, ordererURL = '', ordererOrgName = '') {
    if (!name) {
    ...
    if (!ordererOrgName && !(this.orderer && this.orderer.orgnization)) {
    ...
    const channelReq = this.__constructNodeRequest('/channel/create', this.RequestTypes.POST, {
      name,
      ordererOrgName: ordererOrgName || this.orderer.orgnization,
  ...
  async instantiateChaincode({ name, channelName, functionName, args, endorsementPolicy }) {
    if (!Array.isArray(args)) {
      throw new Error('Chaincode instantiate error : args should be a json array');
    }
    if (!channelName) {
