...
class Paymeter {
   ...
   * @param {string(hex)} [options.functionCallData] Raw transaction data to call functions of smart contract. Only applicable for ETH wallets
   ...
   * @memberof Paymeter
   */
  async send({ fromWalletId, toAddress, amount, password, feeWalletId, feeWalletPassword, feeCollectWalletId, tokenValueInEth, sendExactAmount, functionCallData }) {
    if (!fromWalletId) {
      return Promise.reject(new Error('walletId is a required option'));
    }
    ...
    if (functionCallData) {
      body.options.functionCallData = functionCallData;
    }
