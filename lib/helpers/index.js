...
function getHost() {
  return process.env.BLOCKCLUSTER_PLATFORM_HOST || `https://${process.env.NODE_ENV === 'test' ? 'test' : 'app'}.blockcluster.io`;
}
...
const helpers = {
  ...
  sendPlatformRequest: async ({ domain, path, method, body, apiKey }) => {
    const options = {
      uri: `${domain || getHost()}${path}`,
  ...
  txnsMined: async (txns, web3, config) => {
    ...
    const allReceipts = async () => {
      ...
      try {
        const receiptsData = await Promise.all(receipts);
        const data = JSON.parse(await helpers.sendRequest(config, ''));
        receiptsData.forEach(receipt => {
          if (data.blockToScan - 1 < receipt.blockNumber) {
